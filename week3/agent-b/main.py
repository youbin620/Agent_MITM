...
app = Flask(__name__)
TOOL_URL = os.environ.get("TOOL_URL", "http://127.0.0.1:5100")

# ✅ Burp 프록시 설정 (환경변수에서 읽어오기)
PROXY_URL = os.environ.get("AGENT_PROXY")  # 예: http://127.0.0.1:8080
...
def process():
    ...
    # ✅ Tool Server 호출 (여기에 proxies + 예외 처리 추가)
    try:
        r = requests.post(
            f"{TOOL_URL}/tool",
            json=tool_call,
            timeout=5,
            proxies=PROXIES,  # ← Burp 프록시
        )
        r.raise_for_status()  # 4xx/5xx면 예외 발생
        tool_result = r.json()
    except Exception as e:
        # ❗ 여기서 반드시 JSON으로 에러를 돌려줘야 A에서 json()이 안 터짐
        return jsonify(
            {
                "type": "error",
                "error": f"tool-server 호출 실패: {e}",
                "debug": {
                    "tool_url": TOOL_URL,
                    "proxy_url": PROXY_URL,
                },
    ...
    return jsonify(
        {
            ...
            "debug": {"prompt": prompt, "action": action, "tool_call": tool_call},
        }
    )
